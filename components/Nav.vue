<template>
    <div>
        <nav class="grid grid-cols-2 md:pe-4 lg:pe-8 border border-1 border-white items-center fixed website-margins left-0 right-0 top-5 text-white navbar" ref="navbar">
            ...
            <!-- <div class="hidden lg:flex gap-16 text-slate-200 justify-end">
                <RouterLink v-for="navItem in navItems" :key="navItem.name" :to="navItem.link" active-class="active">
                    {{ navItem.name }}
                </RouterLink>
            </div> -->
            <!-- <div class="lg:hidden flex justify-end pe-3">
                <button @click="showNavDropdown = !showNavDropdown" class="px-3 py-2 border border-1 rounded">
                    <i v-if="!showNavDropdown" class="fa-solid fa-bars"></i>
            ...
            <div class="md:hidden flex justify-end pe-3">
                <button @click="showNavDropdown = !showNavDropdown"   type="button" class="flex justify-end items-center border border-1 rounded p-2 h-10 justify-center text-sm text-gray-500 rounded-lg md:hidden hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-gray-200 dark:text-gray-400 dark:hover:bg-gray-700 dark:focus:ring-gray-600" >
                    ...
                    <svg v-if="!showNavDropdown" class="w-5 h-5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 17 14">
            ...
            <div class="w-full" id="navbar-default">
             ...
             <!-- <div v-if="!showNavDropdown">
                this is cra cra
             </div>
             {{ showNavDropdown }}
            <div class="lg:hidden flex flex-col gap-5  bg-[#09041C] text-white p-5 rounded-lg shadow-lg">
                motor bike
                <RouterLink v-for="navItem in navItems" :key="navItem.name" :to="navItem.link" active-class="active">
                    {{ navItem.name }}  
                </RouterLink>
